...
export function FocusRoom() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const nextSemester = await requestJson<SemesterDashboard>("/api/v1/semester/dashboard");
      setSemester(nextSemester);
      if (!nextSemester.selected_queue_id) {
        setSession(null);
        setBlock(null);
        return;
      }
      ...
      const sessions = await requestJson<FocusSession[]>(
        `/api/v1/semester-queues/${nextSemester.selected_queue_id}/focus-sessions`,
      );
      const active = sessions.find((item) => item.status === "active") ?? null;
      setSession(active);
      setBlock(
        active && nextSemester.next_action?.id === active.block_id
          ? nextSemester.next_action
          : active
            ? nextSemester.next_action
            : null,
      );
  ...
  useEffect(() => {
    if (!session) return;
    const key = `studyos:focus-room:${session.id}:scratchpad`;
  ...
  const courseHref = useMemo(() => {
    if (!block?.course_id) return null;
    return `/courses/${block.course_id}`;
  }, [block?.course_id]);
  ...
  if (finished) {
    return (
      <main className="focus-room focus-room-state">
        ...
        <a className="focus-room-primary-link" href="/">Return to Today</a>
        <style jsx global>{focusRoomStyles}</style>
      </main>
    );
  }
  ...
  if (!session || !block) {
    return (
      <main className="focus-room focus-room-state">
        ...
        <a className="focus-room-primary-link" href="/">Go to Today</a>
  ...
  return (
    <div className="focus-room">
      <header className="focus-room-topbar">
        <a href="/" className="focus-room-brand" aria-label="Back to StudyOS Today">
          <BrandMark />
          <strong>StudyOS<span>.</span></strong>
        </a>
        <div className="focus-room-live"><i /> Focus session live</div>
        <a href="/" className="focus-room-exit">Exit room</a>
      </header>
          ...
          <div className="focus-room-tools">
            ...
            {courseHref && (
              <div className="focus-room-tool-links">
                <a href={`${courseHref}?tab=sources`}><UiIcon name="sources" /><span><strong>Sources</strong><small>Open the course material</small></span></a>
                <a href={`${courseHref}?tab=tutor`}><UiIcon name="tutor" /><span><strong>Practice</strong><small>Work this topic deeply</small></span></a>
                <a href={`${courseHref}?tab=cheats`}><UiIcon name="courses" /><span><strong>Cheat sheet</strong><small>Review compressed notes</small></span></a>
              </div>
            )}
          </div>
        </section>
...
const focusRoomStyles = `
  .focus-room {
    ...
    min-height: 100vh;
    background:
      radial-gradient(circle at 38% -10%, #b6ebce0b, transparent 34%),
      radial-gradient(circle at 110% 70%, #6f9f8508, transparent 35%),
      var(--fr-bg);
    color: var(--fr-text);
  }
  ...
  .focus-room-live i {
    ...
    box-shadow: 0 0 0 6px #b6ebce0c;
    animation: focus-room-pulse 2s ease-in-out infinite;
  }
  ...
  .focus-room-stage {
    min-width: 0;
    padding: clamp(52px, 6vw, 96px) clamp(34px, 6vw, 94px) 72px;
  ...
  .focus-room-clock {
    ...
    border-radius: 16px;
    background:
      radial-gradient(circle at 50% 50%, #b6ebce08, transparent 43%),
      linear-gradient(145deg, #111714, #0d100f);
    box-shadow: inset 0 1px #ffffff04, 0 32px 100px #0000002e;
  }
  ...
  .tutor-panel textarea:focus { border-color: #4a6557; box-shadow: 0 0 0 3px #b6ebce08; }

  .tutor-panel form { display: grid; gap: 9px; }
  .tutor-panel form button {
    min-height: 38px;
    display: inline-flex;
    align-items: center;
  ...
  @media (max-width: 640px) {
    ...
    .focus-room-stage h1 { font-size: 47px; }
