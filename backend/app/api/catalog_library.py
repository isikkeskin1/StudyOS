...
def _catalog_read(db: Session, item: CatalogCourse) -> CatalogCourseRead:
    ...
    count = (
        db.scalar(select(func.count(Document.id)).where(Document.course_id == source.id))
        or 0
    )
...
def _folder(db: Session, item: CatalogCourse, folder_id: str) -> CatalogFolder:
    ...
    if folder is None or folder.catalog_course_id != item.id:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Library folder not found",
        )
    return folder
...
def _breadcrumbs(
    ...
    while cursor is not None:
        if cursor.id in seen or len(chain) >= 64:
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT,
                detail="Invalid library folder hierarchy",
            )
        seen.add(cursor.id)
        chain.append(cursor)
...
def _catalog_document(
    ...
    invalid = (
        document is None
        or document.course_id != item.source_course_id
        or document.status != "processed"
    )
    if invalid:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
...
@router.get(
    "/admin/catalog/courses/{catalog_id}/library",
    response_model=CatalogLibraryRead,
)
...
def create_folder(
    ...
    if not name or name in {".", ".."} or "/" in name or "\\" in name:
        raise HTTPException(
            ...
            detail="Enter a valid folder name",
        )
    folder = CatalogFolder(
        catalog_course_id=item.id,
...
async def upload_library_document(
    ...
    if duplicate is not None:
        ...
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="This document is already in the institutional course",
        )
...
def read_library_document(
    ...
    if not path.is_file():
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Stored library file is missing",
        )
    ...
    with zipfile.ZipFile(archive, mode="w", compression=zipfile.ZIP_DEFLATED) as bundle:
        for document in selected:
            ...
            if archive_path in used_paths:
                ...
                renamed = base.with_name(
                    f"{base.stem}-{document.id[:8]}{base.suffix}"
                )
                archive_path = str(renamed).replace("\\", "/")
            used_paths.add(archive_path)
            bundle.write(path, archive_path)
    archive.seek(0)
    catalog_name = _catalog_read(db, item).name
    safe_name = "".join(
        character if character.isalnum() or character in {"-", "_"} else "-"
        for character in catalog_name
    ).strip("-")
    safe_name = safe_name or "studyos-library"
    return StreamingResponse(
        archive,
        media_type="application/zip",
        headers={
            "Content-Disposition": f'attachment; filename="{safe_name}-pack.zip"'
        },
